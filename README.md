...
This protocol enables multiple AI assistants (Pi, Claude Code, Codex, Cursor, etc.) to collaboratively brainstorm, debate, and document solid technical designs. The output is a standardized markdown document that a developer with zero context can implement from — with tests that validate the implementation.
...
│   └── examples/
│       └── todo-app-workshop.md # Full example transcript
├── scripts/
...
**Total:** ~2-3 hours for a medium-complexity system

## The BLOCK / CONSENT / CONCERN Protocol

Any agent can raise flags at any time:

- **BLOCK** `[section] — [reason]` → Halts progress. Facilitator decides: valid (backtrack), invalid (override), or partial (fix subsection).
- **CONCERN** `[section] — [reason]` → Non-blocking risk, recorded but doesn't halt.
- **CONSENT** `[section] — [agent name]` → Explicit endorsement, tracked for consensus tally.
...
See `docs/examples/todo-app-workshop.md` for a complete realistic transcript.

## Version

**v1.0.0** — Initial release with 8-phase protocol, prompt template library, role bank, and BLOCK/CONSENT/CONCERN mechanics.
